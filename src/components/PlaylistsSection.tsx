...
export function PlaylistsSection({ type, playlists, columns, limit, tracks }: PlaylistsSectionProps) {
  ...
  return (
    <ListOrGridSection type={type} title="Playlists" columns={columns}>
      <PlaylistLikedTracksItem type={type} key="likedSongs" tracks={tracks} />
      {items.map((playlist) => (
